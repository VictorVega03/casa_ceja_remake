...
{
    public static async Task ShowMessageDialog(
        Window parentWindow,
        string title,
        string message,
        AppDialogTone? tone = null)
    {
        await AppDialog.ShowMessageAsync(parentWindow, title, message, tone);
    }
    ...
    {
        return await TextInputDialog.ShowAsync(parentWindow, title, prompt, defaultValue);
    }
    ...
    {
        return await AppDialog.ShowConfirmAsync(
            parentWindow,
            title,
            message,
            yesText,
    ...
    {
        var result = await FileConflictDialog.ShowAsync(parentWindow, fileName);
        return result switch
        {
            FileConflictResult.Replace => DuplicateFileAction.Replace,
            FileConflictResult.Duplicate => DuplicateFileAction.Duplicate,
            _ => DuplicateFileAction.Cancel
        };
    }
